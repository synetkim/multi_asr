...
if [ ! -f $working_dir/attendlstm.fine.done ]; then
  ...
  $cmd $working_dir/log/attendlstm.fine.log \
    ...
    python pdnn/cmds/run_ATTEND_LSTM.py --train-data "$working_dir/data.$dataset/train.*.scp,partition=3000m,random=false,stream=true,target_dim=$outfeat_dim,extra_dim=0" \
...
function forward()
{
    ...
    export PYTHONPATH=$PYTHONPATH:`pwd`/pdnn/ \; \
    export THEANO_FLAGS=mode=FAST_RUN,device=$gpu,floatX=float32 \; \
	python pdnn/cmds2/run_ATTENDLSTMFeat.py --in_scp_file $tmpdir/input.$set.$n.scp \

...
from models.cnnv_extra import CNNV
...
if __name__ == '__main__':
    ...
    required_arguments = ['extra_nnet_spec', 'train_data', 'valid_data', 'nnet_spec', 'conv_nnet_spec', 'wdir']
    ...
    cnn = CNNV(numpy_rng=numpy_rng, theano_rng = theano_rng, cfg = cfg)
    # load the pre-training networks, if any, for parameter initialization
    if (ptr_layer_number > 0) and (resume_training is False):
        _file2nnet(cnn.layers,cnn.extra_layers,  set_layer_num = ptr_layer_number, filename = ptr_file)
    if resume_training:
        _file2nnet(cnn.layers,cnn.extra_layers, filename = wdir + '/nnet.tmp')

    # get the training, validation and testing function for the model
    log('> ... getting the finetuning functions')
    train_fn, valid_fn = cnn.build_finetune_functions(
                (cfg.train_x, cfg.train_y), (cfg.valid_x, cfg.valid_y), (cfg.extra_train_x), (cfg.extra_valid_x),
    ...
    while (cfg.lrate.get_rate() != 0):
        ...
        log('> epoch %d, training error %f ' % (cfg.lrate.epoch, numpy.mean(train_error)))
        # validation 
        ...
        if cfg.lrate.epoch % cfg.model_save_step == 0:
            _nnet2file(cnn.layers, cnn.extra_layers, filename=wdir + '/nnet.tmp')
            _lrate2file(cfg.lrate, wdir + '/training_state.tmp')

    # save the model and network configuration
    if cfg.param_output_file != '':
        _nnet2file(cnn.layers, cnn.extra_layers, filename=cfg.param_output_file, input_factor = cfg.input_dropout_factor, factor = cfg.dropout_factor)

...
from models.lstmv import LSTMV
...
if __name__ == '__main__':
    ...
    cfg = NetworkConfig();cfg.model_type = 'ATTEND'
    ...
    while (cfg.lrate.get_rate() != 0):
        ...
        if numpy.isnan(numpy.mean(train_error)) == True:
            ...
            break           
        log('> epoch %d, training error %0.4f ' % (cfg.lrate.epoch, numpy.mean(train_error)) )
        # validation 
        valid_error = validate_by_minibatch(valid_fn, cfg)
        log('> epoch %d, lrate %f, validation error %0.4f ' % (cfg.lrate.epoch, cfg.lrate.get_rate(), numpy.mean(valid_error)))
        ...
        if cfg.lrate.epoch % cfg.model_save_step == 0:
            _nnet2file(nn.layers, filename=wdir + '/nnet.tmp')
            _lrate2file(cfg.lrate, wdir + '/training_state.tmp') 

    # save the model and network configuration
    if cfg.param_output_file != '':
        _nnet2file(nn.layers, filename=cfg.param_output_file, input_factor = cfg.input_dropout_factor, factor = cfg.dropout_factor)
        log('> ... the final PDNN model parameter is ' + cfg.param_output_file)
    if cfg.cfg_output_file != '':
        _cfg2file(nn.cfg, filename=cfg.cfg_output_file)
    ...
    if cfg.kaldi_output_file != '':
        nn.write_model_to_kaldi(cfg.kaldi_output_file)

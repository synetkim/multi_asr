...
class NetworkConfig():
    ...
    def __init__(self):
        ...
        self.extra_dim = None
        self.extra_train_sets = None
        self.extra_train_x = None
        self.extra_valid_sets = None
        self.extra_valid_x = None
        ...
        # specifically for LSTM 
        self.lstm_layers_sizes = []
    ...
    def init_data_reading_test(self, data_spec):
        print 'init_data_reading_test: '+str(data_spec)
    ...
    def parse_config_extra(self, arguments, nnet_spec):
        extra_layers = nnet_spec.split(':')
        self.extra_layers_sizes = [int(extra_layers[i]) for i in range(0, len(extra_layers))]
    ...
    def parse_config_ldnn(self, arguments, nnet_spec, lstm_nnet_spec):
        self.parse_config_common(arguments)
        # parse LSTM network structure
        lstm_layers = lstm_nnet_spec.split(':')
        self.n_ins = int(lstm_layers[0])
        self.lstm_layers_sizes = [int(lstm_layers[i]) for i in range(1, len(lstm_layers))]
        # parse DNN network structure
        fc_layers = nnet_spec.split(':')
        self.hidden_layers_sizes = [int(fc_layers[i]) for i in range(0, len(fc_layers)-1)]
        self.n_outs = int(fc_layers[-1])

    def parse_config_attend(self, arguments, nnet_spec, lstm_nnet_spec):
        self.parse_config_common(arguments)
        # parse LSTM network structure
        lstm_layers = lstm_nnet_spec.split(':')
        self.n_ins = int(lstm_layers[0])

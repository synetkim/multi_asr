...
class CRNV(object):
    ...
    def __init__(self, numpy_rng, theano_rng=None, cfg = None, testing = False, input = None):
        ...
        #self.fc_dnn = DNNV(numpy_rng=numpy_rng, theano_rng = theano_rng, cfg = self.cfg, input  = self.layers[-1].output)
        self.fc_dnn = RNNV(numpy_rng=numpy_rng, theano_rng = theano_rng, cfg = self.cfg, input  = self.layers[-1].output)
        ...
        self.finetune_cost = self.fc_dnn.logLayer.l2(self.y)

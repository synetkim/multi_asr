...
if __name__ == '__main__':
    ...
    lstm = PhaseATTEND_LSTM(numpy_rng=rng, theano_rng=theano_rng, cfg = cfg, draw=1)
    ...
    while True:
        ...
        for index in xrange(in_matrix.shape[0]/cfg.batch_size):
            final_matrix[index*cfg.batch_size:(index+1)*cfg.batch_size] = out_function(in_matrix[index*cfg.batch_size:(index+1)*cfg.batch_size],extra_in_matrix[index*cfg.batch_size:(index+1)*cfg.batch_size])
        if remainder > 0:
            print '\tremainder:'+str(remainder)
            final_matrix[-remainder:] = out_function(in_matrix[-remainder:],extra_in_matrix[-remainder:])

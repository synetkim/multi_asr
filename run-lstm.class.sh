...
pdnndir=`pwd`/pdnn  
dataset=multi
working_dir=`pwd`/exp_pdnn.enhan.$dataset
...
if [ ! -f $working_dir/lstm.fine.done ]; then
  ...
  $cmd $working_dir/log/lstm.fine.log \
    ...
    python pdnn/cmds/run_LSTM.py --train-data "$working_dir/data.$dataset/train.*.scp,partition=3000m,random=false,stream=true,target_dim=$outfeat_dim,extra_dim=0" \
                                    --valid-data "$working_dir/data.$dataset/valid.*.scp,random=false,stream=true,target_dim=$outfeat_dim,extra_dim=0" \
...
function forward()
{
    ...
    export PYTHONPATH=$PYTHONPATH:`pwd`/pdnn/ \; \
    export THEANO_FLAGS=mode=FAST_RUN,device=$gpu,floatX=float32 \; \
	python pdnn/cmds2/run_LSTMFeat.py --in_scp_file $tmpdir/input.$set.$n.scp \
...
for set in dt05_real et05_real; do
    ...
    outdir=$working_dir/data-new/${set}_dnnenhan/
   	steps_pdnn/decode_dnn_from_pdf.sh --nj 4 --cmd "$decode_cmd" \
   		$graph_dir $outdir/ ${gmmdir}_ali $working_dir/decode.lstm.$dataset.$set &
    sleep 3;
done
wait;

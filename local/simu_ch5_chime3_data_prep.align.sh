...
echo "extract 5th channel (CH5.wav, the center bottom edge in the front of the tablet) for noisy5 data"
...
if $eval_flag; then
list_set="tr05_simu_noisy5 dt05_simu_noisy5 et05_simu_noisy5"
else
list_set="tr05_simu_noisy5 dt05_simu_noisy5"
fi
...
cat tr05_simu_noisy5_wav.scp | awk -F'[_]' '{print $2}' | tr '[A-Z]' '[a-z]' \
    | $local/find_noisy_transcripts.pl dot_files.flist | cut -f 2- -d" " > tr05_simu_noisy5.txt
cat tr05_simu_noisy5_wav.scp | cut -f 1 -d" " > tr05_simu_noisy5.ids
paste -d" " tr05_simu_noisy5.ids tr05_simu_noisy5.txt | sort -k 1 > tr05_simu_noisy5.trans1
# dt05 and et05 simulation data are generated from the CHiME 3 booth recording
# and we use CHiME 3 dot files
cat dt05_simu.dot | sed -e 's/(\(.*\))/\1/' | awk '{print $NF ".CH5_SIMU"}'> dt05_simu_noisy5.ids
cat dt05_simu.dot | sed -e 's/(.*)//' > dt05_simu_noisy5.txt
paste -d" " dt05_simu_noisy5.ids dt05_simu_noisy5.txt | sort -k 1 > dt05_simu_noisy5.trans1
if $eval_flag; then
cat et05_simu.dot | sed -e 's/(\(.*\))/\1/' | awk '{print $NF ".CH5_SIMU"}'> et05_simu_noisy5.ids
cat et05_simu.dot | sed -e 's/(.*)//' > et05_simu_noisy5.txt
paste -d" " et05_simu_noisy5.ids et05_simu_noisy5.txt | sort -k 1 > et05_simu_noisy5.trans1
fi

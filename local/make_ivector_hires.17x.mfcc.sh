...
if [ $stage -le 1 ]; then
  for datadir in et05_real_${enhan} et05_simu_${enhan} tr05_real_${enhan} tr05_simu_${enhan} tr05_simu_wsj1_${enhan} dt05_real_${enhan} dt05_simu_${enhan}; do
    utils/copy_data_dir.sh data/$datadir data/${datadir}_hires
    steps/make_mfcc.sh --nj $nj --mfcc-config conf/mfcc_hires.conf \
      --cmd "$train_cmd" data/${datadir}_hires exp/make_hires/$datadir $mfccdir || exit 1;
    steps/compute_cmvn_stats.sh data/${datadir}_hires exp/make_hires/$datadir $mfccdir || exit 1;
  done
  utils/combine_data.sh data/tr05_multi_${enhan}_hires data/tr05_simu_${enhan}_hires data/tr05_real_${enhan}_hires data/tr05_simu_wsj1_${enhan}_hires

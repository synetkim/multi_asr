...
if [ $# -ne 2 ]; then
  printf "\nUSAGE: %s <corpus-directory> <numCH>\n\n" `basename $0`
...
if $eval_flag; then
list_set="et05_simu_noisy${numCH}"
fi

cd $dir

if $eval_flag; then
find $audio_dir -name "*CH${numCH}.wav" | grep 'et05_bus_simu\|et05_caf_simu\|et05_ped_simu\|et05_str_simu' | sort -u > et05_simu_noisy${numCH}.flist
fi
...
if $eval_flag; then
cat et05_simu.dot | sed -e 's/(\(.*\))/\1/' | awk -v ch=${numCH} '{print $NF ".CH"ch"_SIMU"}'> et05_simu_noisy${numCH}.ids
cat et05_simu.dot | sed -e 's/(.*)//' > et05_simu_noisy${numCH}.txt
paste -d" " et05_simu_noisy${numCH}.ids et05_simu_noisy${numCH}.txt | sort -k 1 > et05_simu_noisy${numCH}.trans1
fi

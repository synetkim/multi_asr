...
echo "extract 4th channel (CH4.wav, the center bottom edge in the front of the tablet) for noisy4 data"
...
if $eval_flag; then
list_set="tr05_simu_noisy4 dt05_simu_noisy4 et05_simu_noisy4"
else
list_set="tr05_simu_noisy4 dt05_simu_noisy4"
fi

cd $dir

find $audio_dir -name '*CH4.wav' | grep 'tr05_bus_simu\|tr05_caf_simu\|tr05_ped_simu\|tr05_str_simu' | sort -u > tr05_simu_noisy4.flist
find $audio_dir -name '*CH4.wav' | grep 'dt05_bus_simu\|dt05_caf_simu\|dt05_ped_simu\|dt05_str_simu' | sort -u > dt05_simu_noisy4.flist
if $eval_flag; then
find $audio_dir -name '*CH4.wav' | grep 'et05_bus_simu\|et05_caf_simu\|et05_ped_simu\|et05_str_simu' | sort -u > et05_simu_noisy4.flist
fi
...
cat tr05_simu_noisy4_wav.scp | awk -F'[_]' '{print $2}' | tr '[A-Z]' '[a-z]' \
    | $local/find_noisy_transcripts.pl dot_files.flist | cut -f 2- -d" " > tr05_simu_noisy4.txt
cat tr05_simu_noisy4_wav.scp | cut -f 1 -d" " > tr05_simu_noisy4.ids
paste -d" " tr05_simu_noisy4.ids tr05_simu_noisy4.txt | sort -k 1 > tr05_simu_noisy4.trans1
# dt05 and et05 simulation data are generated from the CHiME 3 booth recording
# and we use CHiME 3 dot files
cat dt05_simu.dot | sed -e 's/(\(.*\))/\1/' | awk '{print $NF ".CH4_SIMU"}'> dt05_simu_noisy4.ids
cat dt05_simu.dot | sed -e 's/(.*)//' > dt05_simu_noisy4.txt
paste -d" " dt05_simu_noisy4.ids dt05_simu_noisy4.txt | sort -k 1 > dt05_simu_noisy4.trans1
if $eval_flag; then
cat et05_simu.dot | sed -e 's/(\(.*\))/\1/' | awk '{print $NF ".CH4_SIMU"}'> et05_simu_noisy4.ids
cat et05_simu.dot | sed -e 's/(.*)//' > et05_simu_noisy4.txt
paste -d" " et05_simu_noisy4.ids et05_simu_noisy4.txt | sort -k 1 > et05_simu_noisy4.trans1
fi

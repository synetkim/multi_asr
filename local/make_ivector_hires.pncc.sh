...
if [ $stage -le 1 ]; then
  ...
  for datadir in et05_real_${enhan} dt05_real_${enhan} ; do
    ...
    steps/make_pncc_mat_etdt.real.sh --nj $nj --pncc_config conf/pncc_mat_hires.conf \
      --cmd "$train_cmd" data/${datadir}_hires exp/make_hires/$datadir $pncc_matdir || exit 1;
    steps/compute_cmvn_stats.sh data/${datadir}_hires exp/make_hires/$datadir $pncc_matdir || exit 1;
  done
  for datadir in et05_simu_${enhan} dt05_simu_${enhan}; do
    utils/copy_data_dir.sh data/$datadir data/${datadir}_hires
    steps/make_pncc_mat_etdt.simu.sh --nj $nj --pncc_config conf/pncc_mat_hires.conf \
      --cmd "$train_cmd" data/${datadir}_hires exp/make_hires/$datadir $pncc_matdir || exit 1;
    steps/compute_cmvn_stats.sh data/${datadir}_hires exp/make_hires/$datadir $pncc_matdir || exit 1;
  done
  for datadir in tr05_real_${enhan} tr05_simu_${enhan}; do
    utils/copy_data_dir.sh data/$datadir data/${datadir}_hires
    steps/make_pncc_mat.sh --nj $nj --pncc_config conf/pncc_mat_hires.conf \
      --cmd "$train_cmd" data/${datadir}_hires exp/make_hires/$datadir $pncc_matdir || exit 1;
    steps/compute_cmvn_stats.sh data/${datadir}_hires exp/make_hires/$datadir $pncc_matdir || exit 1;
  done

...
echo "extract 1th channel (CH1.wav, the center bottom edge in the front of the tablet) for noisy data"
...
if $eval_flag; then
list_set="tr05_real_noisy1 dt05_real_noisy1 et05_real_noisy1"
else
list_set="tr05_real_noisy1 dt05_real_noisy1"
fi

cd $dir

find $audio_dir -name '*CH1.wav' | grep 'tr05_bus_real\|tr05_caf_real\|tr05_ped_real\|tr05_str_real' | sort -u > tr05_real_noisy1.flist
find $audio_dir -name '*CH1.wav' | grep 'dt05_bus_real\|dt05_caf_real\|dt05_ped_real\|dt05_str_real' | sort -u > dt05_real_noisy1.flist
if $eval_flag; then
find $audio_dir -name '*CH1.wav' | grep 'et05_bus_real\|et05_caf_real\|et05_ped_real\|et05_str_real' | sort -u > et05_real_noisy1.flist
fi
...
cat tr05_real.dot | sed -e 's/(.*)//' > tr05_real_noisy1.txt
paste -d" " tr05_real_noisy1.ids tr05_real_noisy1.txt | sort -k 1 > tr05_real_noisy1.trans1
cat dt05_real.dot | sed -e 's/(\(.*\))/\1/' | awk '{print $NF ".CH1_REAL"}'> dt05_real_noisy1.ids
cat dt05_real.dot | sed -e 's/(.*)//' > dt05_real_noisy1.txt
paste -d" " dt05_real_noisy1.ids dt05_real_noisy1.txt | sort -k 1 > dt05_real_noisy1.trans1
if $eval_flag; then
cat et05_real.dot | sed -e 's/(\(.*\))/\1/' | awk '{print $NF ".CH1_REAL"}'> et05_real_noisy1.ids
cat et05_real.dot | sed -e 's/(.*)//' > et05_real_noisy1.txt
paste -d" " et05_real_noisy1.ids et05_real_noisy1.txt | sort -k 1 > et05_real_noisy1.trans1
fi

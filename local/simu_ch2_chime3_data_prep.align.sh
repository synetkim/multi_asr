...
echo "extract 2th channel (CH2.wav, the center bottom edge in the front of the tablet) for noisy2 data"
...
if $eval_flag; then
list_set="tr05_simu_noisy2 dt05_simu_noisy2 et05_simu_noisy2"
else
list_set="tr05_simu_noisy2 dt05_simu_noisy2"
fi

cd $dir

find $audio_dir -name '*CH2.wav' | grep 'tr05_bus_simu\|tr05_caf_simu\|tr05_ped_simu\|tr05_str_simu' | sort -u > tr05_simu_noisy2.flist
find $audio_dir -name '*CH2.wav' | grep 'dt05_bus_simu\|dt05_caf_simu\|dt05_ped_simu\|dt05_str_simu' | sort -u > dt05_simu_noisy2.flist
if $eval_flag; then
find $audio_dir -name '*CH2.wav' | grep 'et05_bus_simu\|et05_caf_simu\|et05_ped_simu\|et05_str_simu' | sort -u > et05_simu_noisy2.flist
fi
...
cat tr05_simu_noisy2_wav.scp | awk -F'[_]' '{print $2}' | tr '[A-Z]' '[a-z]' \
    | $local/find_noisy_transcripts.pl dot_files.flist | cut -f 2- -d" " > tr05_simu_noisy2.txt
cat tr05_simu_noisy2_wav.scp | cut -f 1 -d" " > tr05_simu_noisy2.ids
paste -d" " tr05_simu_noisy2.ids tr05_simu_noisy2.txt | sort -k 1 > tr05_simu_noisy2.trans1
# dt05 and et05 simulation data are generated from the CHiME 3 booth recording
# and we use CHiME 3 dot files
cat dt05_simu.dot | sed -e 's/(\(.*\))/\1/' | awk '{print $NF ".CH2_SIMU"}'> dt05_simu_noisy2.ids
cat dt05_simu.dot | sed -e 's/(.*)//' > dt05_simu_noisy2.txt
paste -d" " dt05_simu_noisy2.ids dt05_simu_noisy2.txt | sort -k 1 > dt05_simu_noisy2.trans1
if $eval_flag; then
cat et05_simu.dot | sed -e 's/(\(.*\))/\1/' | awk '{print $NF ".CH2_SIMU"}'> et05_simu_noisy2.ids
cat et05_simu.dot | sed -e 's/(.*)//' > et05_simu_noisy2.txt
paste -d" " et05_simu_noisy2.ids et05_simu_noisy2.txt | sort -k 1 > et05_simu_noisy2.trans1
fi

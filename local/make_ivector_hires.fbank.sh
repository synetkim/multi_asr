...
if [ $stage -le 1 ]; then
  for datadir in et05_real_${enhan} et05_simu_${enhan} tr05_real_${enhan} tr05_simu_${enhan} dt05_real_${enhan} dt05_simu_${enhan}; do
    ...
    steps/make_fbank.sh --nj $nj --fbank-config conf/fbank_hires.conf \
      --cmd "$train_cmd" data/${datadir}_hires exp/make_hires/$datadir $fbankdir || exit 1;
    steps/compute_cmvn_stats.sh data/${datadir}_hires exp/make_hires/$datadir $fbankdir || exit 1;
  done
...
if [ $stage -le 7 ]; then
  rm exp/nnet2_online/.error 2>/dev/null
    steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj 4 \
      data/dt05_multi_${enhan}_hires exp/nnet2_online/extractor exp/nnet2_online/ivectors_dt05_multi_${enhan} || touch exp/nnet2_online/.error &

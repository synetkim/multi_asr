...
for x in dt05_real_$enhan ; do
  ...
  steps/nnet/make_fmllr_ivector_feats.sh --cmd "$train_cmd" \
     ...
     $dir data/$x $gmmdir $dir/log exp/nnet2_online/ivectors_dt05_real_$enhan $dir/data || exit 1
done
...
for x in tr05_real_$enhan ; do
  ...
  steps/nnet/make_fmllr_ivector_feats.sh --cmd "$train_cmd" \
     ...
     $dir data/$x $gmmdir $dir/log exp/nnet2_online/ivectors_tr05_real_$enhan $dir/data || exit 1
done

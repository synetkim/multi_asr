...
if $eval_flag; then
list_set="et05_real_noisy${numCH}"
fi

cd $dir

if $eval_flag; then
find $audio_dir -name "*CH${numCH}.wav" | grep 'et05_bus_real\|et05_caf_real\|et05_ped_real\|et05_str_real' | sort -u > et05_real_noisy${numCH}.flist
fi
...
if $eval_flag; then
cat et05_real.dot | sed -e 's/(\(.*\))/\1/' | awk -v ch=${numCH} '{print $NF ".CH"ch"_REAL"}'> et05_real_noisy${numCH}.ids
cat et05_real.dot | sed -e 's/(.*)//' > et05_real_noisy${numCH}.txt
paste -d" " et05_real_noisy${numCH}.ids et05_real_noisy${numCH}.txt | sort -k 1 > et05_real_noisy${numCH}.trans1
fi

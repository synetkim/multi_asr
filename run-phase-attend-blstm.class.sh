...
if [ ! -f $working_dir/phaseattendlstm.fine.done ]; then
  ...
  $cmd $working_dir/log/phaseattendlstm.fine.log \
    ...
    python pdnn/cmds/run_Phase_ATTEND_BLSTM.py --train-data "$working_dir/data.$dataset/$phase_data_id.extra_with_train.*.scp,partition=3000m,random=false,stream=true,target_dim=$outfeat_dim,extra_dim=$phase_dim" \
...
function forward()
{
    ...
    export PYTHONPATH=$PYTHONPATH:`pwd`/pdnn/ \; \
    export THEANO_FLAGS=mode=FAST_RUN,device=$gpu,floatX=float32 \; \
    python pdnn/cmds2/run_PhaseATTENDBLSTMFeat.py --in_scp_file $tmpdir/input.$set.$n.scp \

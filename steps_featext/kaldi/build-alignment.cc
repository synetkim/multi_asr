...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    BaseFloatMatrixWriter kaldi_writer(feature_wspecifier); 
    ...
    for (; !feature_reader.Done(); feature_reader.Next()) {
      ...
      KALDI_LOG << "key: " <<key <<", num_utt:"<< num_utt ;
      Matrix<BaseFloat> outputMat;
      outputMat.Resize(alignment.size(), 1);
      for (int i = 0; i < alignment.size(); i++) {
        KALDI_LOG << "alignments:" << alignment[i]<<", pdf:" << trans_model.TransitionIdToPdf(alignment[i]);
        outputMat(i,0) = trans_model.TransitionIdToPdf(alignment[i]);
      } 
      kaldi_writer.Write(feature_reader.Key(), outputMat);
      num_done ++; num_utt ++;
    }
    //ko.Close();

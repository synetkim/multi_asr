...
void ComputePowerSpectrumPhaseDifference( 
										VectorBase<BaseFloat> *base_waveform,
										VectorBase<BaseFloat> *waveform1,
										VectorBase<BaseFloat> *waveform3,
										VectorBase<BaseFloat> *waveform4,
										VectorBase<BaseFloat> *waveform5,
										VectorBase<BaseFloat> *waveform6,
										VectorBase<BaseFloat> *result
										) {
  int32 dim = waveform1->Dim();
  int32 half_dim = dim/2;
  KALDI_LOG << "ComputePowerSpectrumPhaseDifference: dim: "<< dim; // 512
  KALDI_LOG << "ComputePowerSpectrumPhaseDifference: half_dim: "<< half_dim; //256
  ...
  SubVector<BaseFloat> local_base_waveform(*base_waveform, 0, dim);
  SubVector<BaseFloat> local_waveform1(*waveform1, 0, dim);
  SubVector<BaseFloat> local_waveform3(*waveform3, 0, dim);
  SubVector<BaseFloat> local_waveform4(*waveform4, 0, dim);
  SubVector<BaseFloat> local_waveform5(*waveform5, 0, dim);
  SubVector<BaseFloat> local_waveform6(*waveform6, 0, dim);
 
  for (int32 i = 1; i < half_dim; i++) {
    // FFT Coeff
    int32 idx_real=i*2;
    int32 idx_imag=i*2+1;
    BaseFloat real0 = (local_base_waveform)(idx_real);
    BaseFloat im0   = (local_base_waveform)(idx_imag);
    BaseFloat real1 = (local_waveform1)(idx_real);
    BaseFloat im1   = (local_waveform1)(idx_imag);
    BaseFloat real3 = (local_waveform3)(idx_real);
    BaseFloat im3   = (local_waveform3)(idx_imag);
    BaseFloat real4 = (local_waveform4)(idx_real);
    BaseFloat im4   = (local_waveform4)(idx_imag);
    BaseFloat real5 = (local_waveform5)(idx_real);
    BaseFloat im5   = (local_waveform5)(idx_imag);
    BaseFloat real6 = (local_waveform6)(idx_real);
    BaseFloat im6   = (local_waveform6)(idx_imag);
   
    // Phase Difference
    BaseFloat angle  = atan2(im0,real0);
    BaseFloat angle1 = atan2(im1,real1);
    BaseFloat angle3 = atan2(im3,real3);
    BaseFloat angle4 = atan2(im4,real4);
    BaseFloat angle5 = atan2(im5,real5);
    BaseFloat angle6 = atan2(im6,real6);
    (*result)(half_dim*0+i) = ComputePD(angle, angle1);
    (*result)(half_dim*1+i) = ComputePD(angle, angle3);
    (*result)(half_dim*2+i) = ComputePD(angle, angle4);
    (*result)(half_dim*3+i) = ComputePD(angle, angle5);
    (*result)(half_dim*4+i) = ComputePD(angle, angle6);
  }
  (*result)(half_dim*0) = 0; 
  (*result)(half_dim*1) = 0; 
  (*result)(half_dim*2) = 0; 
  ...
  (*result)(half_dim*4) = 0; 
  //KALDI_LOG << *result;
  KALDI_LOG << "Done copying phase to result vector: 0-" << half_dim*4+half_dim-1;
}

BaseFloat ComputePD(BaseFloat angle1, BaseFloat angle2) {
    // 1. abs(angle(1) - angle(2))
    BaseFloat diff1 = std::abs(angle1 - angle2);
    // 2. abs(angle(1) - angle(2) - 2*pi)
    BaseFloat diff2 = std::abs(angle1 - angle2 - M_2PI);
    // 3. abs(angle(1) - angle(2) + 2*pi)
    BaseFloat diff3 = std::abs(angle1 - angle2 + M_2PI);
	return std::min(std::min(diff1, diff2), diff3);
}

void ComputePowerSpectrumPhase(VectorBase<BaseFloat> *waveform) {
  ...
  // now we have in waveform, first half of complex spectrum
  // it's stored as [real0, realN/2-1, real1, im1, real2, im2, ...]
  SubVector<BaseFloat> local_waveform(*waveform, 0, dim);
  int32 half_dim = dim/2;
  BaseFloat first_energy = (*waveform)(0) * (*waveform)(0), last_energy = (*waveform)(1) * (*waveform)(1);
  for (int32 i = 1; i < half_dim; i++) {
    BaseFloat real = (local_waveform)(i*2), im = (local_waveform)(i*2 + 1);
    (*waveform)(i) = real*real + im*im;
    (*waveform)(i+half_dim) = atan(im/real);
  }
  (*waveform)(0) = first_energy;
  (*waveform)(half_dim) = last_energy; // first phase is 0
}

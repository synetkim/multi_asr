...
if [ ! -f $working_dir/lstm.fine.done ]; then
  echo "Fine-tuning BLSTMV"
  $cmd $working_dir/log/lstm.fine.log \
    export PYTHONPATH=$PYTHONPATH:`pwd`/pdnn/ \; \
    export THEANO_FLAGS=mode=FAST_RUN,device=$gpu,floatX=float32 \; \
    python pdnn/cmds/run_BLSTM.py --train-data "$working_dir/data.$dataset/train.*.scp,partition=3000m,random=false,stream=true,target_dim=$outfeat_dim,extra_dim=0" \
...
function forward()
{
    ...
    export PYTHONPATH=$PYTHONPATH:`pwd`/pdnn/ \; \
    export THEANO_FLAGS=mode=FAST_RUN,device=$gpu,floatX=float32 \; \
	python pdnn/cmds2/run_BLSTMFeat.py --in_scp_file $tmpdir/input.$set.$n.scp \

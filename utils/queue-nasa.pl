...
# queue-nasa.pl has the same functionality as run.pl, except that
# it runs the job in question on the queue (Sun GridEngine).
# This version of queue-nasa.pl uses the task array functionality
# of the grid engine.  Note: it's different from the queue-nasa.pl
...
# command qsub -v PATH -S /bin/bash -j oe -l arch=*64*
...
sub print_usage() {
  print STDERR
   "Usage: queue-nasa.pl [options] [JOB=1:n] log-file command-line arguments...\n" .
   "e.g.: queue-nasa.pl foo.log echo baz\n" .
   " (which will echo \"baz\", with stdout and stderr directed to foo.log)\n" .
   "or: queue-nasa.pl -q all.q\@xyz foo.log echo bar \| sed s/bar/baz/ \n" .
   " (which is an example of using a pipe; you can provide other escaped bash constructs)\n" .
   "or: queue-nasa.pl -q all.q\@qyz JOB=1:10 foo.JOB.log echo JOB \n" .
...
for (my $x = 1; $x <= 3; $x++) { # This for-loop is to 
  ...
  if ($ARGV[0] =~ m/^([\w_][\w\d_]*)+=(\d+):(\d+)$/) { # e.g. JOB=1:20
    ...
    shift;
    if ($jobstart > $jobend) {
      die "queue-nasa.pl: invalid job range $ARGV[0]";
    }
    if ($jobstart <= 0) {
    ...
    $jobstart = $2;
    $jobend = $jobstart + 1;
    $jobstep = 2; # Hack to trick PBS to run a single job array
    shift;
  } elsif ($ARGV[0] =~ m/.+\=.*\:.*$/) {
    print STDERR "Warning: suspicious first argument to queue-nasa.pl: $ARGV[0]\n";
  }
}
...
while(<CONFIG>) {
  ...
  if ($_ =~ /^command (.+)/) {
    ...
    print STDERR "queue-nasa.pl: unable to parse line '$line' in config file ($config)\n";
    exit(1);
  }
}
...
if ($read_command != 1) {
  print STDERR "queue-nasa.pl: config file ($config) does not contain the line \"command .*\"\n";
  exit(1);
}
...
for my $option (keys %cli_options) {
  ...
  if (exists $cli_default_options{($option,$value)}) {
    ...
    die "queue-nasa.pl: Command line option $option not described in $config (or value '$value' not allowed)\n";
  }
}
...
if ($array_job == 1 && $logfile !~ m/$jobname/
    && $jobend > $jobstart && $jobstep == 1) {
  print STDERR "queue-nasa.pl: you are trying to run a parallel job but "
...
# we pass as arguments to queue-nasa.pl, such as "|", we want to be
...
if ($array_job == 1) { # It's an array job.
  $queue_array_opt = "-J $jobstart-$jobend:$jobstep"; 
...
if ($ret != 0) {
  if ($sync && $ret == 256) { # this is the exit status when a job failed (bad exit status)
    ...
    print STDERR "queue-nasa.pl: job writing to $logfile failed\n";
  } else {
    print STDERR "queue-nasa.pl: error submitting jobs to queue (return status was $ret)\n";
...
if (! $sync) { # We're not submitting with -sync y, so we
  ...
  if (!defined $jobname) { # not an array job.
    ...
    for (my $jobid = $jobstart; $jobid <= $jobend; $jobid+=$jobstep) {
  ...
  { # Get the PBS job-id from the log file in q/
    ...
    undef $pbs_job_id;
    my $row = <L>;
    # The first line should contain the JOB ID and nothing else in the HECC cluster;
    if (index($row, '.nas.nasa.gov') != -1) {
  ...
  foreach my $f (@syncfiles) {
	...
	# $tmpoutput = `qstat $pbs_job_id 2>&1`;
	# sleep(3);
	# if (index($tmpoutput, 'Job has finished') != -1) {
	#   # The PBS finished executing before the first check itself: OK.
	#     system("touch $f");
	#     next;
	# }
	$ret = system("qstat $pbs_job_id >/dev/null 2>/dev/null");
        ...
        if ($ret>>8 == 1) {     # Job does not seem to exist
          ...
          if ($last_line =~ m/status 0$/ && (-M $logfile) < 0) {
            ...
            print STDERR "**queue-nasa.pl: syncfile $f was not created but job seems\n" .
              "**to have finished OK.  Probably your file-system has problems.\n" .
              "**This is just a warning.\n";
            last;
          } else {
            chop $last_line;
            print STDERR "queue-nasa.pl: Error, unfinished job no " .
          ...
          print STDERR "queue-nasa.pl: Warning: qstat command returned status $ret (qstat $pbs_job_id,$!)\n";
        }
      }
    }
  }
...
if ($num_failed == 0) { exit(0); }
  ...
  if (@logfiles == 1) {
    ...
    print STDERR "queue-nasa.pl: job failed with status $status, log is in $logfile\n";
    if ($logfile =~ m/JOB/) {
      print STDERR "queue-nasa.pl: probably you forgot to put JOB=1:\$nj in your script.\n";
    }
  } else {
    if (defined $jobname) { $logfile =~ s/\$PBS_ARRAY_INDEX/*/g; }
    my $numjobs = 1 + $jobend - $jobstart;
    print STDERR "queue-nasa.pl: $num_failed / $numjobs failed, log is in $logfile\n";
  }
  exit(1);
}

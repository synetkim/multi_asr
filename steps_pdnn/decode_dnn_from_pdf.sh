...
echo "sykim: options srcdir: $srcdir"
ls $srcdir
thread_string=
...
#echo "sykim: options splice: $splice_opts, delta: $add_deltas, norm: $norm_vars"
mkdir -p $dir/log
...
echo "sykim: class.counts + pdfs => finalfeats"
feats="copy-feats scp:$sdata/JOB/feats.scp ark:- |"
finalfeats="$feats ./steps_featext/kaldi/nnet-forward-from-pdfs --class-frame-counts=$dir/class.counts --apply-log=true --no-softmax=false $srcdir/dnn.nnet ark:- ark:- |"

$cmd JOB=1:$nj $dir/log/nnet-forward-from-pdfs.JOB.log \
./steps_featext/kaldi/nnet-forward-from-pdfs --class-frame-counts=$dir/class.counts --apply-log=true --no-softmax=false $srcdir/dnn.nnet scp:$sdata/JOB/feats.scp ark:$sdata/JOB/feats.nnet-forward-from-pdfs.JOB.ark

$cmd JOB=1:$nj $dir/log/decode.JOB.log \
  latgen-faster-mapped --max-active=$max_active --beam=$beam --lattice-beam=$latbeam --acoustic-scale=$acwt --allow-partial=true --word-symbol-table=$graphdir/words.txt $alidir/final.mdl $graphdir/HCLG.fst ark:$sdata/JOB/feats.nnet-forward-from-pdfs.JOB.ark "ark:|gzip -c > $dir/lat.JOB.gz" || exit 1;

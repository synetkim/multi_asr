...
echo "$0: network inputs have the dimension of $feat_dim $feats"

if [ $stage -le 2 ]; then
  $cmd JOB=1:$nj $dir/log/build_pfile.$name.JOB.log \
    forKaldi/build-pfile-from-ali-noise-label-auto --every-nth-frame=$every_nth_frame $alidir/final.mdl "ark:gunzip -c $alidir/ali.JOB.gz|" \
  ...
  if $do_concat; then
    ...
    $cmd JOB=1:$nj $dir/log/gzip.$name.JOB.log gzip -f $dir/$name.pfile.JOB || exit 1;
  fi
fi

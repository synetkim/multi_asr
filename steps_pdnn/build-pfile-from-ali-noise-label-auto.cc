...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    int32 noise_label = 0;
    po.Register("every-nth-frame", &noise_label, "This option will cause it to print "
    ...
    for (; !feature_reader.Done(); feature_reader.Next()) {
      ...
      std::string str2 ("PED");
      std::string str3 ("STR");
      if (key.find(str0) != string::npos) {
        noise_label=0;
      } 
      else if (key.find(str1) != string::npos) {
        noise_label=1;
      } 
      else if (key.find(str2) != string::npos) {
        noise_label=2;
      } 
      else if (key.find(str3) != string::npos) {
        noise_label=3;
      } 
      else {
        KALDI_WARN << "Did not find key for utterance, key: " << key << ", noise_label:" << noise_label;
      } 
